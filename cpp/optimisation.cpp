...
long long int computepartCost(long int *s, int start) {
    int h,k;
    long long int sum;
    for (sum = 0, h = start; h < PSize; h++ )
	for ( k = h + 1; k < PSize; k++ )
	    sum += CostMat[s[h]][s[k]];
    return sum;
}
...
void cheneryWatanabe(long int *s){
    ...
    long int* empty;
    int cost;
    empty = (long int *)malloc(PSize * sizeof(long int)); // keep track of which rows have been used and which are still usable
